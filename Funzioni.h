...
byte LeggiPulsante(byte& Verso, unsigned long& Timer ){	
  ...
  if(digitalRead (Button_up)==0)f_btnUp=true;	//Legge lo stato del pulsante allГўв‚¬в„ўuscita per aggiornare il flag
  else f_btnUp=false;
  
  if(digitalRead (Button_dn)==0)f_btnDn=true;	//Legge lo stato del pulsante allГўв‚¬в„ўuscita per aggiornare il flag
  else f_btnDn=false;
